...
void editorUpdateRow(erow *row) {
   ...
   /* Create a version of the row we can directly print on the screen,
    ...
    if (allocsize > UINT32_MAX) {
        printf("Some line of the edited file is too long for sedit\n");
        exit(1);
    }
...
#define SEDIT_QUERY_LEN 256

void editorFind(int fd) {
    char query[SEDIT_QUERY_LEN+1] = {0};
    int qlen = 0;
...
void editorProcessKeypress(int fd) {
    ...
    quit_times = SEDIT_QUIT_TIMES; /* Reset it to the original value. */
}
...
int EndsWithSn(const char *str) { return EndsWith(str, ".sn"); }


int main(int argc, char **argv) {
    if (argc != 2) {
        fprintf(stderr,"Usage: sedit [file]\n");
        exit(1);
    }
